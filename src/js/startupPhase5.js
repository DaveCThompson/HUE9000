...
export async function createPhaseTimeline(dependencies) {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            // Account for button flickers
            if (diagnosticButtons.length > 0) {
                const lastButtonFlickerStartTime = (diagnosticButtons.length - 1) * configModule.STARTUP_BUTTON_GROUP_APPEAR_STAGGER;
                const buttonFlickerDur = configModule.estimateFlickerDuration('buttonFlickerToDimlyLit');
                phaseDurationSeconds = Math.max(phaseDurationSeconds, lastButtonFlickerStartTime + buttonFlickerDur);
            }

...
class TerminalManager {
    ...
    async concludeScan({ status }) {
        ...
        if (status === 'aborted') {
            appState.emit('requestTerminalMessage', {
                type: 'status', source: 'scan', messageKey: 'SCAN_ABORTED', interrupt: true
            });
        } else if (status === 'error') {
            appState.emit('requestTerminalMessage', {
                type: 'status', source: 'scan', messageKey: 'FSM_ERROR',
    ...
    _interruptCurrentTask() {
        if (this._currentProcessPromise) this._currentProcessPromise.abort = true;
        this._currentProcessPromise = null;
    ...
    _handleRequestTerminalMessage(payload) {
        ...
        if (payload.instant) {
            this._printMessageInstantly({ ...payload, ...getMessage(payload) });
            return;
        }
        ...
        if (messageObject.interrupt) this._interruptAndClear();
    ...
    _printMessageInstantly(messageObject) {
        ...
        if (!this._isFirstLine && messageObject.formatting.spacingBefore > 0) {
            for (let i = 0; i < messageObject.formatting.spacingBefore; i++) this._addNewLineAndPrepareForTyping(true);
        }
        messageObject.content.forEach(lineSegments => {
            ...
            if (this._currentLineElement) {
                lineSegments.forEach(segment => {
                    ...
                    if (segment.styles) span.classList.add(...segment.styles.map(s => `tm-text--${s}`));
    ...
    async _processQueue() {
        ...
        while (this._messageQueue.length > 0) {
            ...
            if (messageObject.beforeTyping && Array.isArray(messageObject.beforeTyping)) {
                for (const command of messageObject.beforeTyping) {
                    ...
                    if (handler) await handler(command.params, processPromise);
                }
            }
            if (processPromise.abort) break;
        ...
        this._isProcessing = false;
        this._currentProcessPromise = null;
        if (this._messageQueue.length === 0 && !this._isTakeoverActive) this._setCursorState('idle');
    }
    ...
    async _typeMessage(messageObject, promise) {
        ...
        for (let i = 0; i < allLogicalLines.length; i++) {
            ...
            if (promise.abort) return;
            if (i < allLogicalLines.length - 1) await this._pauseAndBlink(TERMINAL_INTER_LINE_PAUSE_S, promise);
        }
        this._isFirstLine = false;
    }
    ...
    _addNewLineAndPrepareForTyping(isSpacer = false, className = null) {
        ...
        if (!isSpacer) this._currentLineElement.appendChild(this._cursorElement);
    ...
    async _typeLine(lineSegments, speedPerChar, options = {}, promise) {
        ...
        const segmentSpans = lineSegments.map(segment => {
            const span = document.createElement('span');
            if (segment?.styles?.length) span.classList.add(...segment.styles.map(s => `tm-text--${s}`));
        ...
        for (let i = 0; i < lineSegments.length; i++) {
            ...
            for (const char of segmentText) {
                if (promise.abort) return;
                segmentSpans[i].textContent += char;
    ...
    _scrollTerminal(instant = false) {
        ...
        if (!container) return;
        const isUserAtBottom = container.scrollHeight - container.clientHeight <= container.scrollTop + 50;
        if (isUserAtBottom) {
            this._gsap.to(container, {
                scrollTop: container.scrollHeight, duration: instant ? 0 : TERMINAL_SCROLL_DURATION_S, ease: 'power2.out'
            });
        }
    }
    ...
    async _handleDisplayTextCommand({ text }, promise) {
        if (promise.abort) return;
        await this._typeMessage({ content: [[{ text }]], formatting: { spacingBefore: 0 }, type: 'status' }, promise);
    }
    ...
    async _handleSpinnerCommand({ duration = 1500, text = 'PROCESSING...' }, promise) {
        ...
        spinnerLine.append(spinnerCursor, textNode);
